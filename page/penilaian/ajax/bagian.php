...
// FROM (
//   SELECT *,
//          ROW_NUMBER() OVER(PARTITION BY `id_karyawan_fk_pemilik` ORDER BY `nama_periode_tahun_fk` DESC, `id_periode_bulan_fk` DESC) AS row_num
...
<br>
<table id="example" class="table table-bordered table-striped table-jq">
  <thead>
    <tr>
      <th>No</th>
      <th>Nama karyawan</th>
  ...
  <tbody>
    <?php if ($fetchYear['aksi_tahun'] == "on" && $fetchMonth['aksi_bulan'] == "on") { ?>
      ...
      //$sqlgabungLeft = "SELECT * FROM karyawan LEFT JOIN penilaian_akhir_pemilik ON karyawan.id=penilaian_akhir_pemilik.id_karyawan_fk_pemilik WHERE karyawan.jabatan LIKE '%Staf $type%' AND penilaian_akhir_pemilik.id_periode_bulan_fk='$monthsin' AND penilaian_akhir_pemilik.nama_periode_tahun_fk='$yearsin'";
      ...
      foreach ($query as $row): ?>
        ...
        <tr>
          ...
          <td>
            ...
            <?php } else { ?>
              <a class="btn btn-danger disabled"><i class="glyphicon glyphicon-pencil"></i></a>
            <?php } ?>
          </td>
        </tr>
